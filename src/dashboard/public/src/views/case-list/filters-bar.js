...
const { SearchInput, Select, Chip, Dropdown, Btn } = ds.components;
const h = ds.h;
...
export function StagePills() {
  ...
  const selected = state.filt.status || '';
  return h('div', { class: 'ds-filter-pills ds-stage-pills', role: 'group', 'aria-label': 'Quick stage filter' },
    ...options.map((o) => h('button', {
      key: 'sp-' + o.id,
      type: 'button',
      class: 'ds-filter-pill' + (o.tone ? ' tone-' + o.tone : '') + (o.id === selected ? ' active' : ''),
      'aria-pressed': o.id === selected ? 'true' : 'false',
      onclick: () => setFilt({ status: o.id }),
    }, o.label))
  );
}
