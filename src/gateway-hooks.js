...
export function caseSystemPrompt(caseRow, events, contact, { orient = null } = {}) {
  ...
  return [
    ...
    `If the worker reads out real GPS coordinates directly (numbers from a phone,`,
    `never your own guess), record case_report's lat/lon so the team's map shows`,
    `the exact point -- only when they genuinely give you numbers, never estimated.`,
