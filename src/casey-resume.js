...
export async function resumePendingTurnsBody({ store, log, gateway, adapters, handle }, { maxCases, maxRedrives, spacingMs }) {
  let scanned = 0, resumed = 0
    ...
    for (const c of rows) {
      ...
      // fetching its event log, the cheapest possible short-circuit. The
      // per-msgId cap alone is not enough: a case with MANY historical inbound
      // messages (a contact who sent several messages during an outage) picks a
      // DIFFERENT still-under-cap msgId on each boot's sweep, so the case as a
      // whole never stops being re-driven, and every walk burns a full
      // provider-chain attempt per msgId, competing directly with live traffic
      // for the same rate-limited capacity. A dead-lettered case still shows up
      // in the operator's needs-human queue (tagged below) -- nothing is lost, it
      // just stops silently consuming provider budget forever.
      ...
      // OR previously attempted-but-degraded (no real reply) with retries still
      // under the cap. A msgId marked resume-attempted with NO resume-degraded
      // marker is treated as a genuine, silent completion (the handler threw, or
      // something unexpected happened with no observation recorded) and is not
      // retried: unlike degraded, there is no positive signal here that another
      // attempt would behave differently. RESUME_DEGRADED_RETRY_CAP bounds total
      // retries across all future boots so a permanently-broken backend still
      // stops trying eventually, the same discipline as drainQueuedTurns'
      // retryCap -> queue-drive-failed dead-letter.
      const RESUME_DEGRADED_RETRY_CAP = 5
      // Age ceiling, independent of attempt count: a message that has sat
      // unanswered this long has almost certainly had its sender move on, so
      // continuing to resume-retry it provides no real value and only keeps
      // consuming provider budget shared with live traffic. It bounds the total
      // number of BOOTS a stuck msgId can ever be retried across, which the
      // attempt-count cap alone does not do: a case with many old distinct
      // pending msgIds otherwise keeps finding a fresh under-cap one to retry,
      // boot after boot, indefinitely.
      ...
      if (!pending) {
        // No genuinely-pending msgId survives (each is either completed, a
        // silent non-degraded miss, or capped). If at least one was a REAL
        // exhausted retry (anyCapped), this case will not self-resolve via
        // further resume attempts: dead-letter it so it stops consuming a
        // scan + event-fetch AND, more importantly, never re-enters the pending
        // selection above on a future boot. A case whose only uncompleted
        // msgIds are silent (non-degraded) misses is left alone deliberately --
        // unlike a capped degraded retry, a silent miss carries no signal that
        // resuming would ever have helped, so there is nothing to conclude by
        // giving up.
        if (anyCapped) {
          try {
            await store.updateCase(c.id, { tags: mergeTag(mergeTag(c.tags, 'resume-exhausted'), 'needs-human') })
            // data.dead_lettered, not just the resume-exhausted TAG/prose text,
            // so GET /api/turns/degraded (operations.js filters on structured
            // data fields, never text-prefix parsing) can surface this as its own
            // explicit, queryable terminal state -- distinct from "still queued,
            // will retry" -- rather than an operator only being able to infer
            // dead-letter status from the case no longer being re-driven.
      ...
      // done -- the completion check below appends a SEPARATE
      // resume-degraded:<id> marker when the redrive itself comes back
      // degraded (blanked, no real reply), so a LATER boot's sweep still sees
      // this msgId as pending (resume-attempted alone does not make it
      // completedAfter-equivalent) and gets another shot once the underlying
      // model/provider issue clears. handle.call(...)'s return value MUST be
      // inspected: treating ANY non-throwing call as a permanent success
      // silently abandons a contact whose reply was correctly blanked by the
      // degraded-turn guards and never actually sent.
      // Spaced, not a burst: each re-drive walks the SAME provider chain a
      // brand-new live contact's message would, so firing them back-to-back
      // with no delay competes directly for the same tiny per-minute
      // rate-limit windows a real inbound needs right now -- a genuine live
      // message arriving mid-sweep times out because every configured provider
      // is still rate-limited by the sweep's OWN traffic seconds earlier. For
      // the same reason maxRedrives defaults to 10
      ...
      // testing/restart churn) that must not compound into starving live
      // traffic. Gated on resumed > 0 so the FIRST re-drive of a boot still
      // fires immediately (nothing to wait behind yet); placed here
      // (immediately before the actual re-drive), not earlier in the loop, so
      // cases skipped by the eligibility filters above never pay the delay.
      ...
      // on its own. Split it: container is the FIRST segment, author the LAST.
      // replyTarget() reads msg.raw.channel_id directly, so passing the combined
      // external_id through unsplit sends every resumed reply on a multi-author
      // Discord channel to Discord as an invalid channel id (400 Invalid Form
      // Body, NUMBER_TYPE_COERCE), silently never reaching the contact even when
      // the LLM call itself succeeded. splitExternalId (hooks/handler.js) owns
      // that rule -- read it there before changing either call site.
