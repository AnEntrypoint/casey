...
const ALL_DIMENSIONS = [
  ...
  },
  {
    ...
    invariant: 'A content-free turn must STAY content-free regardless of the channel markup wrapping it. A platform injects markup when a contact addresses the bot -- Discord renders "@memobot hello" as "<@BOTID> hello", "<@!id>", "<@&id>", "<#id>", "<:emoji:id>". That markup must be stripped before capture/intent/content-free reasoning so a bare greeting reads as content-free and drives intake, NOT the case-ack with a fabricated count (the witnessed bug: the mention snowflake digits were captured as affected_count, flipping isContentFreeTurn to false). Check: does the inbound boundary strip every mention/emoji form (user/role/channel, with and without !/&)? Does extractFields independently refuse to read a bracketed-snowflake id as a count (defense in depth)? Does a REAL report after a mention ("<@id> my cattle are dying", "<@id> 5 cows died") still capture species/symptoms/the real count and drive intake? Is the raw inbound still recorded verbatim for audit while only the reasoning copy is cleaned? WhatsApp/web paths must be unaffected (no markup to strip).',
  },
  {
    key: 'drives-collection-every-turn',
    files: 'src/gateway-hooks.js (intakeAdvanceReply ~491, warmConversationalReply ~444, captureDrivenReply ~1319, advancingFallback, mostImportantMissingField/VISIT_CRITICAL_ASK ~1185, detectNewCaseConflict ~55, the content-free branch ~900 and the gate ~961)',
    invariant: 'memobot must DRIVE the conversation to collect everything it needs on EVERY turn, because there is only one point in time when someone is on-site -- a reply must read as "asking for the next needed fact", never the "Thank you for letting us know ... your reference is X" holding-ack and never a no-ask pleasantry. Check: does every intake-driving reply (intakeAdvanceReply and its callers) LEAD with a brief greeting/ack + the ask, with the holding-ack preamble fully removed (the witnessed "@memobot hi -> Thank you for letting us know..." on an existing case)? On a bare greeting (empty case) does the reply ask the first needed fact (where/which animals), and on a populated case the next most-important missing fact? Does intake keep asking the tool-only how_to_find/farmer_available/contact_fallback (in VISIT_CRITICAL_ASK) one per turn, once each, until exhausted -- not stopping at the auto-extractable fields? ESCAPE ROUTE: does detectNewCaseConflict flag a returning contact who states a clearly different species/location (a durable NEW-CASE-SIGNAL observation, since the agent can clobber the needs-split tag) WITHOUT false-triggering on the same outbreak continuing? Do replies stay <=240 chars and language-mirrored, and do the assisted-draft / jargon / ref-correction guards still apply to the new opener?',
  },
]
