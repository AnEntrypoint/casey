...
const CROSSCUTS = [
  ...
  {
    key: 'xc-ingress-trust',
    lens: 'PUBLIC-INGRESS TRUST BOUNDARY (P6 + P10) across the WhatsApp/Discord webhook + receive paths. casey accepts inbound messages from a PUBLIC endpoint, so the ingress is the highest-value attack surface and the least-considered one. Hunt, grounded in real file:line: (1) WEBHOOK SIGNATURE VERIFICATION -- does the WhatsApp/Meta webhook (or any HTTP ingress) verify the X-Hub-Signature-256 HMAC against the app secret before trusting the body? An unverified webhook lets anyone forge inbound farmer messages and drive cases. If verification is absent, that is a high-severity finding; the fix is a constant-time HMAC check that rejects (401) before any case mutation. (2) RATE-LIMIT / ABUSE -- can a single sender or IP flood the public webhook to exhaust the LLM budget, the case store, or the operator inbox? Is there any per-contact or per-IP throttle, or a cap on case creation rate? Name the missing rung. Report ONLY what is grounded; if a contract (Meta webhook handshake) constrains the fix, respect it.',
  },
  {
    key: 'xc-media-capture',
    lens: 'MEDIA-CAPTURE COMPLETENESS (P1 + P12) across the ingress + extraction path. A photo is already captured at ingress (inboundImageNote), but the AUDIO/VOICE-NOTE path is the named open frontier: a low-literacy farmer is far more likely to send a voice note than type. Hunt, grounded in real file:line: (1) is an inbound audio/voice message recorded as explicit case state at all, or silently dropped / labelled only as text? (2) is there any transcription capture hook, even a deferred-to-operator one, so the voice content becomes operator-visible structured state rather than an opaque attachment? (3) does a media-only voice message still get a reply and a case? Propose the MINIMAL non-breaking capture (mirror the photo path: record at ingress, fill-if-empty, never block the reply) -- do NOT propose adding a heavy transcription dependency unless a published primitive already provides it; a deferred operator-transcribe hook is an acceptable honest degradation rung.',
  },
  {
    key: 'xc-ai-tells',
