...
// thatcher and freddie are both consumed by casey from npm and PUBLISH ON PUSH via
// CI, so a change to either reaches casey only after a publish.
//
...
const CONTEXT =
  ...
  'consumes BOTH thatcher and freddie from npm (caret range) and both PUBLISH ON PUSH via CI, so a ' +
  'change to either reaches casey only after a publish. Read sibling ' +
