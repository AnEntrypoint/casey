...
async function main() {
  ...
  if (cmd === 'doctor') {
    ...
    if (existsSync(path.join(ROOT, 'deps', 'freddie', '.git'))) {
      const { execFileSync } = await import('node:child_process')
      try {
        execFileSync(process.execPath, [path.join(ROOT, 'scripts', 'check-submodules.mjs')], { cwd: ROOT, stdio: 'pipe' })
        console.log(ok('submodules: all deps/* on main, clean, up to date'))
      } catch (e) {
        console.log(bad('submodules need attention -- run `npm run check-submodules` for details'))
        problems++
      }
    }
