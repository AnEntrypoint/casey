...
|-------|---------|---------------|
| Agent + channels | [`freddie`](https://github.com/AnEntrypoint/freddie) | Agent harness + Gateway with WhatsApp/Discord adapters, tools, sessions. Drives the agentic behaviour. |
...
editing, and resolved as `github:AnEntrypoint/<repo>#main` npm dependencies
at runtime (never a `file:../` sibling, never CDN-served). A local fix
requires a commit + push to the composed project's own `main` branch (from
inside `deps/<project>`), then `npm install` in casey to pick it up.
