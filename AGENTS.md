...
fix is recorded here too. `thatcher`, `anentrypoint-design`, and `freddie`
still track `latest` in `package.json` deliberately, so a fix pushed
upstream reaches casey's npm-resolved runtime copy on the next `npm
install` with no version-bump commit required for the *runtime* dependency
-- the submodule pointer bump is a separate, additional step for keeping
`deps/` in sync, not a replacement for `npm install`. This matters most for
`anentrypoint-design` and `freddie`, both still on a `0.0.x` line where a
caret range would pin rather than float (`^0.0.391` resolves to exactly
`0.0.391`, never the next patch) -- `latest` is the only way those two
actually track upstream. `thatcher` is past 1.0 (where a caret would
genuinely float within the major) but stays on `latest` anyway, matching the
other two composed projects rather than carving out an exception. `acptoapi`
is the one composed project casey pins with a real caret range (`^1.0.x`),
trading bleeding-edge for a stability floor on the LLM provider chain. A
...
`latest` npm dependency; casey resolves it out of `node_modules` and serves
its `dist/` directly. Browser-delivered consumers load it from unpkg at
`@latest`. Two consumers are deliberately excluded from this strategy and
...
stability). Accepted tradeoff: a kit publish can change casey's dashboard UI
with no commit in casey.
...
casey consumes thatcher via npm `latest`, never a `file:../` sibling --
casey's dependency policy means it is always effectively "a bare npm
install" relative to thatcher's published version, so `case-store.js` calls
thatcher's operator-where directly with no runtime feature-detect and no
fallback. busybase's `src/*.js` are gitignored bun-build outputs -- fixes go
