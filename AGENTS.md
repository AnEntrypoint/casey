...
| `CASEY_MEDIA_TOOL_TIMEOUT_MS` | Bounds each of the three opt-in media-enrichment `dispatchTool` calls (`transcription`/`vision`/`tts` above, `src/hooks/media.js`) with a race timeout (default 12000). None of freddie's dispatch path or the bare `fetch()` calls beneath it carry any timeout of their own, and `transcribeAudio`/`describePhoto` both run BEFORE `turnStartedAt` is set (`hooks/handler.js`), so without this they sit entirely outside `CASEY_TURN_HARD_DEADLINE_MS` -- a half-open connection to the transcription/vision/TTS provider would hang the whole inbound turn and the per-contact concurrency gate indefinitely. A timeout is caught by each function's existing try/catch and degrades exactly like any other media failure (silent fallback to the operator-listens/operator-opens-the-photo/text-only path, never blocks the reply). Kept well under the 60s turn hard-deadline since these three effectively steal from that same budget. |
| `CASEY_TRUST_PROXY_HOPS` | Sets express's `trust proxy` config to this many hops (`dashboard/server.js`). Default unset = off (`req.ip` is the raw socket peer, matching today's behavior for a direct/ngrok/dev deployment). Set to the real number of trusted reverse-proxy/load-balancer hops in front of a production deployment, or the public `/report` form's rate limiter (`reportRateLimited`, `dashboard/routes/auth.js`) sees every request's socket peer as the SAME proxy address and collapses every farmer into one shared 10-req/60s bucket -- any combination of 10 legitimate submissions in a minute 429s every other farmer. Set too high/untrusted, a client can spoof `X-Forwarded-For` to bypass the limiter -- know your own real proxy chain depth before setting this. |
| `CASEY_MINE_SCAN_LIMIT` | Bounds `case_mine`/`case_today`'s (`case-tools.js` `mineRows`) system-wide open-case scan window before JS-filtering to a worker's own (default 1000). No thatcher-level query can scope by owner directly -- `thatcher.config.yml` declares no `row_access` field for `case` (only `assignee`), and `ownsCase`'s `container:author` containment semantics are not expressible via thatcher's `$gte/$lte/$in/$or` operators, only exact equality -- so this is a scan-window mitigation, not a real fix: on a high-traffic deployment where more contacts than this limit have more recent activity than a given worker's own open cases, "my cases" can still read empty for a genuinely open case. |
...
| `CASEY_TURN_HARD_DEADLINE_MS` | Guaranteed-response FSM (`hooks/handler.js`): the total wall-clock budget (default 60000, raised to 90000 in this deployment's `.env`) a LIVE first-attempt turn's retry loop may spend before the turn is closed out with a guaranteed fallback message instead of continuing to retry. Each individual attempt still gets to run its own bounded provider-chain walk (acptoapi's `ACPTOAPI_CHAIN_LINK_TIMEOUT_MS` below, 30s per hop in this deployment) to real completion rather than being cut off mid-hop -- a retry that starts with real remaining budget gets a genuine chance. Never applies to a background resume/queue re-drive. Raised 2026-07-23 alongside `ACPTOAPI_AUTO_CHAIN_CAP` being lowered from 20 to 10 (see below) -- the two settings pace together: several of the extra-providers aggregator pool's genuinely-working models are "thinking"/reasoning models needing 5-25s to actually respond, and a 60s deadline against a 20-candidate pool was killing the whole turn before the walk could ever reach one of them, even though it would have worked. |
...
| `ACPTOAPI_AUTO_CHAIN_CAP` | acptoapi's own setting (read from casey's `.env`, not a `CASEY_`-prefixed var): caps how many candidate models a per-turn `auto` chain build considers (acptoapi default 12). Live-witnessed real degraded turns where two providers each took one transient failure close together, correctly triggering acptoapi's exponential provider-level backoff (up to 8 minutes at its ceiling) -- which exhausted the 12-slot pool on backed-off models before the walk ever reached a provider that was actually healthy, even though healthy providers existed; they just weren't among that turn's 12 candidates. A distinct, more specific root cause of the same symptom (sampler_backoff cascade exhausting the chain) was fixed upstream in acptoapi `1.0.184`: the codestral brand's live model catalog was enumerated from `api.mistral.ai/v1/models` (documented exception, since codestral's own models endpoint 404s) but 56-58 of those ~60 entries are not actually chat-servable through `codestral.mistral.ai/v1/chat/completions` (403 `tier_not_allowed` for non-codestral-branded ids, 400 `invalid_model` for the two embeddings-only ids) -- each guaranteed-fail attempt cascaded `sampler_backoff` across the rest of the same-prefix pool. Fixed by filtering the codestral catalog down to the 2 genuinely chat-servable ids before caching (acptoapi's `lib/brand-catalog.js` `MODELS_FILTER`); raising `ACPTOAPI_AUTO_CHAIN_CAP` alone would not have fixed this specific cause since the poisoned candidates were guaranteed-fail regardless of pool size. Raised to 20 2026-07-something, then LOWERED to 10 on 2026-07-23 after live-witnessing the opposite failure mode: a 20-candidate pool routinely could not be fully walked within `CASEY_TURN_HARD_DEADLINE_MS`'s budget once several genuinely-slow-but-working reasoning models (see `ACPTOAPI_CHAIN_LINK_TIMEOUT_MS` below) were in the mix -- the outer deadline killed the whole turn mid-walk before reaching a link that would have answered. 10 paired with the 90s deadline above leaves real route-around room for a couple of simultaneous hiccups while still being reachable in full within the budget. |
| `ACPTOAPI_CHAIN_LINK_TIMEOUT_MS` | acptoapi's own per-link chain timeout (`chain-machine.js DEFAULT_LINK_TIMEOUT_MS`, default 10000). Raised to 30000 in this deployment's `.env` 2026-07-23: the extra-providers aggregator pool (`tokenhun.shadw.app`) includes several genuinely-working "thinking"/reasoning models (deepseek-v4-*, kimi-k2.x/k3, glm-5, and the two hidden-model ids `ep-gmv4flash`/`ep-zz2xjij6` -- present on this aggregator but absent from its own `/v1/models` listing, only reachable because they're declared explicitly in `~/.acptoapi/extra-providers.txt`) whose hidden reasoning tokens routinely push a real, correct response past 10s (5-25s measured directly against the live endpoint) -- the old 10s timeout was killing these before they could answer. Paced together with `ACPTOAPI_AUTO_CHAIN_CAP` and `CASEY_TURN_HARD_DEADLINE_MS` above so the chain can both AFFORD to wait out a slow-but-working model and still finish its full candidate walk inside the outer deadline. |
